...
if(isset($_POST['cmdsave_tab1']) && $_POST['txt_name']!='') {
	...
	$result = SysEdit('tbl_channels', $arr, " id=".$GetID);

	if($result) $_SESSION['flash'.'com_'.COMS] = 1;
	else $_SESSION['flash'.'com_'.COMS] = 0;
}

$res_Cate = SysGetList('tbl_channels', array(), ' AND id='. $GetID);
if(count($res_Cate) <= 0){
	echo 'Không có dữ liệu.'; 
	return;
}
				...
				<form name="frm_action" id="frm_action" action="" method="post" enctype="multipart/form-data">
					...
					<div class="row">
						...
						<div class="col-md-6">
							<div class="form-group">
								<label>Link</label>
								<input type="text" name="txt_link" class="form-control" value="<?php echo $row['link'];?>" placeholder="Đường dẫn kênh">
							</div>
						</div>
					</div>
