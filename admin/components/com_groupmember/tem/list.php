...
if($isAdmin==1){
	...
	<div class="widget-tree" style="padding-left: 7px;">
		<header class="header">
			<h2><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp&nbsp<?php echo LANG['COM_GMEMBER']['ADD_TITLE'];?></h2>
		</header>
	</div>
				...
				<form id="frm_add" class="smart-form" method="post" action="" style="padding: 15px 10px;">
					...
					<div class="rows">
						<div class="col-md-12 form_wrap_tool">
							<a href="#" onclick="AddGroupMember()" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i><?php echo LANG['COM_GMEMBER']['FRM_ADD'];?></a>
	...
	<script>
		function AddGroupMember(){
			if(frm_validate()){
				var _url="<?php echo ROOTHOST;?>ajaxs/gmember/process_add_gmember.php";
				var _data={
					'par_id': $('#cbo_par').val(),
					'name': $('#txt_name').val(),
					'intro': $('#txt_desc').val()
