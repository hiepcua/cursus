...
if($isAdmin==1){
	...
	<div class="widget-tree" style="padding-left: 7px;">
		<header class="header">
			<h2><i class="fa fa-plus-circle" aria-hidden="true"></i>&nbsp&nbsp<?php echo LANG['COM_GMEMBER']['COM_TITLE'];?></h2>
		</header>
	</div>
	...
	<script>
		...
		function frm_validate(){
			...
			if(name == '') {
				flag = false;
				$('.mess').html('Category name not be empty.');
			}
			if(par_id == 0){
				flag = false;
				$('.mess').html('Category may not be blank.');
			}
			return flag;
		};
