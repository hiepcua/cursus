<?php
function isLogin(){
	if(isset($_SESSION['CURSUS_USER_LOGIN']['status']) && $_SESSION['CURSUS_USER_LOGIN']['status']=='yes'){
...
function getInfo($field){
	$info=isset($_SESSION['CURSUS_USER_LOGIN']['data'][$field])?$_SESSION['CURSUS_USER_LOGIN']['data'][$field]:'N/a';
	return $info;
}
function setInfo($field,$val){
	if(isset($_SESSION['CURSUS_USER_LOGIN']))$_SESSION['CURSUS_USER_LOGIN']['data'][$field]=$val;
}
function checkExpires($user){
	...
	// get session login
	$now=time();
	if(isset($_SESSION['CURSUS_USER_LOGIN']) && $now-$_SESSION['CURSUS_USER_LOGIN']['data']['action_time']>=ACTION_TIMEOUT){
		$obj=new CLS_MYSQL;
		$sql="SELECT session FROM tbl_user_login WHERE username='$user' AND isactive=1 ORDER BY id DESC";
		$obj->Query($sql);
		if($obj->Num_rows()>0){
			$r=$obj->Fetch_Assoc();
			if($_SESSION['CURSUS_USER_LOGIN']['data']['session']!=$r['session']){
				LogOut($user);
				return true;
			}
		}else{
	...
	if(isset($_SESSION['CURSUS_USER_LOGIN']) && $now-$_SESSION['CURSUS_USER_LOGIN']['data']['action_time']>=USER_TIMEOUT){
		LogOut($user);
	}
	$_SESSION['CURSUS_USER_LOGIN']['data']['action_time']=$now;
	return false;
}
function LogIn($user,$pass){
	...
	$_SESSION['CURSUS_USER_LOGIN']=$arr;
	return $arr;
}
function LogOut($user){
	if(isset($_SESSION['CURSUS_USER_LOGIN'])){
		unset($_SESSION['CURSUS_USER_LOGIN']);
