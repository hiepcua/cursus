<?php
function isTeacher($user,$code){
	$object=SysGetList('tbl_class_member',array()," AND username='$user' AND class_code='$code'");
	if(count($object)>0){
		if($object[0]['mtype']=='teach') return true; 
		else return false; 
	}else{
		return false;
	}
...
function setInfo($field,$val){
	if(isset($_SESSION['MEMBER_LOGIN'][$field])) $_SESSION['MEMBER_LOGIN'][$field]=$val;
}
